...
const startBot = async () => {
  ...
  console.log(chalk.green(' > Versi   : ') + chalk.white('1.0.0'));
...
const messageHandler = async (sock, msg) => {
  ...
  if (command) logger.command(command, `${senderName} (${sender.split('@')[0]})`);

  if (!isCmd) {
    const listDb = loadListDb();
    if (isGroupMsg && listDb[msg.key.remoteJid]?.[body.toLowerCase()]) {
      const listItem = listDb[msg.key.remoteJid][body.toLowerCase()];
      await sock.sendMessage(msg.key.remoteJid, {
        [listItem.image && fs.existsSync(listItem.image) ? 'image' : 'text']: listItem.image ? { url: listItem.image, caption: listItem.content } : listItem.content
      }, { quoted: msg });
      return;
    }
  }
  ...
  switch (command) {
    ...
    case 'list': {
      ...
      Object.keys(listDb[msg.key.remoteJid]).forEach((item, index) => {
...
*Catatan:* Gunakan . atau tanpa prefix
© ${config.name} - ${moment().format('YYYY')}
    ...
    default:
      if (isCmd) await sock.sendMessage(msg.key.remoteJid, { text: `❌ "${command}" tidak dikenal, cek .menu` }, { quoted: msg });
      break;
  }
};
